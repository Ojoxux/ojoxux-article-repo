...
こんにちは！最近 X で見かけた ArkRegex という正規表現を型安全に扱えるライブラリについて紹介したいと思います。
...
ArkRegex は、公式によると**`new RegExp()`の型安全なドロップイン置き換え**とのこと。
...
実は私も、正規表現で型ガードを書いていたときに、型定義と正規表現が同期しなくなってバグを踏んだ経験があります。
同じファイルに書いてあるのに、正規表現を修正したら型定義の更新を忘れていて...というパターンです。

Reddit のコメントでも似たような経験をした方がいて、「型ガードと型が同じファイルにあるのに、すでに数回バグった。正規表現から型を導出できたら最高」と書いていました。
まさにそれを実現するのが ArkRegex なんですね。
...
TypeScript は 5.9 以上を推奨していますが、5.1 以上なら動きます。
あと、`package.json`に`"type": "module"`を追加して ES Modules を有効にしておく必要があります。
...
使っていて「あれ？」と思ったことをまとめておきます。

### `[a-Z]`みたいな範囲が厳密に推論されない

文字クラスで範囲指定すると、全部のパターンは推論されません。
これは組み合わせが爆発的に増えて型が巨大になりすぎるからだそうです。

公式によると「性能と精度のバランスを取りつつ、最悪でも大雑把になることはあっても、誤った型にはならないようにしている」とのこと。
まあ確かに、全パターン列挙されても困りますしね。

### 長すぎる正規表現だとエラーになる

めちゃくちゃ長くて複雑な正規表現を書くと、`Type is excessively deep...`というエラーが出ることがあります。
TypeScript の型推論が追いつかないんですね。

その場合は`regex.as`を使って手動で型を指定できます。
...
まあ、そこまで複雑な正規表現を書く機会はあまりないと思いますが...

### 信頼性は大丈夫？

これは気になるところですよね。
公式によると、[attest](https://github.com/arktypeio/arktype/tree/main/ark/attest#readme)というツールで[広範にテスト](https://github.com/arktypeio/arktype/tree/main/ark/regex/__tests__/regex.test.ts)されていて、[ベンチマーク](https://github.com/arktypeio/arktype/tree/main/ark/regex/__tests__/regex.bench.ts)も取られているそうです。

もし変な挙動があったら、[GitHub で issue](https://github.com/arktypeio/arktype/issues/new)を立てれば対応してくれるみたいです。
...
ArkType については[こちらの記事](https://zenn.dev/forcia_tech/articles/202504_hatano_arktype)で詳しく紹介されているので、興味がある方はぜひ。
